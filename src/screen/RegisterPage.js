...
class RegisterScreen extends Component {
  ...
  onBtnRegisterClick = () => {
    if(this.inputEmail && this.state.confirm && this.state.pass){
      ...
      this.setState({error:'Data Kosong Harap Isi'})
    }
  ...
  render() {
    const confirm = this.state.confirm == '' ? <Item floatingLabel last>
      ...
      <Label>Confirm Password</Label>
      <Input onChangeText={(val) => this.setState({ confirm: val })} /><Icon name='close-circle'/>
    </Item> : <Item success floatingLabel last>
          <Label>Confirm Password</Label>
          <Input onChangeText={(val) => this.setState({ confirm: val })} /><Icon name='checkmark-circle'/>
        </Item>
